...
- **Mission Control Report**: 최소 7개 이상의 로그 데이터를 기반으로 심층 분석 제공
  - **상관관계 분석**: Gravity와 Stability 간의 피어슨 상관계수 및 구간별 통계
  - **시간대 분석**: 새벽/아침/오후/밤 시간대별 로그 패턴 분석
  - **키워드 클라우드**: High Gravity(70% 이상) 상태에서 자주 사용하는 단어 추출 및 시각화
  - **회복 탄력성 지수**: Stability 저점에서 회복까지 걸리는 평균 시간 측정 및 트렌드 분석
- **Smart Filtering**: `[Query]` 시스템 질문 제외, 사용자 답변만 분석
...
│   └── Analytics.jsx    # 심층 분석 대시보드
├── hooks/
│   └── useInnerOrbit.js # 비즈니스 로직 & 상태 관리
├── utils/
│   └── analytics.js     # 분석 유틸리티 (상관관계, 시간대, 키워드, 회복 탄력성)
...
- **v1.7.0** (2024-12-05): Advanced Analytics Dashboard (Mission Control Report)
  - 상관관계 분석 (Correlation Analysis)
  - 시간대 패턴 분석 (Temporal Patterns)
  - 키워드 클라우드 (Keyword Cloud)
  - 회복 탄력성 지수 (Resilience Index)
...
- [ ] Export to PDF
- [x] Advanced analytics dashboard ✅ (v1.7.0)
